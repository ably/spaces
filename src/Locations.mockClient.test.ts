...
describe('Locations (mockClient)', () => {
  ...
  describe('set', () => {
    ...
    it<SpaceTestContext>('sends a presence update on location set', async ({ client, space, presence }) => {
    ...
    it<SpaceTestContext>('fires an event when a location is set', async ({ client, space }) => {
      const spy = vi.fn();
      await space.enter();
      space.locations.on("locationUpdate", spy);
      space.locations["onPresenceUpdate"](createPresenceMessage('update', { clientId: '2', connectionId: '2', data: { location: "location2" } }))
      expect(spy).toHaveBeenCalledOnce();
    });

    it<SpaceTestContext>('correctly sets previousLocation', async ({ client, space, presence }) => {
      const spy =  vi.fn();
      await space.enter();
      space.locations.on("locationUpdate", spy);
      space.locations["onPresenceUpdate"](createPresenceMessage('update', { clientId: '2', connectionId: '2', data: { location: "location1" } }))
      space.locations["onPresenceUpdate"](createPresenceMessage('update', { clientId: '2', connectionId: '2', data: { location: "location2" } }))
