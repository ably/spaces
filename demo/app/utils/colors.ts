...
const twColorSet = fullConfig.theme.colors;
const filteredTwSet = Object.keys(twColorSet)
  .filter(
    (colorName) =>
      !['gray', 'zinc', 'stone', 'slate', 'amber', 'neutral'].includes(colorName) &&
      typeof twColorSet[colorName] === 'object',
  )
  .map((colorName) => {
    return {
      colorName,
      intesity: Object.entries(twColorSet[colorName]).filter(([key]) => !['50', '100', '900', '950'].includes(key)),
    };
  });
