...
const renderComments = () => {
  ...
  commentData.forEach((comment, index) => {
    ...
    const commentThreadComment = commentThreadFragment.querySelector('p[data-id=comment-thread-comment]') as HTMLElement;
    const commentThreadPosition = commentThreadFragment.querySelector('p[data-id=comment-thread-position]') as HTMLElement;
    ...
    const initials = comment.name.split(' ').map((name) => name[0]).join('');
